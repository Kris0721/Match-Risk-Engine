...
fn trades(events: &[EngineEvent]) -> Vec<&EngineEvent> {
    events.iter().filter(|e| matches!(e, EngineEvent::Trade { .. })).collect()
}
...
fn no_cross_rests_on_book() {
    ...
    let evs = book.apply(new_limit(1, ACCT_A, Side::Buy, 10000, 10)).into_vec();
...
fn full_fill_crossing_order() {
    ...
    let evs = book.apply(new_limit(2, ACCT_B, Side::Buy, 10010, 5)).into_vec();

    let t = trades(&evs);
    assert_eq!(t.len(), 1, "exactly one trade");
    if let EngineEvent::Trade { qty, price, maker_acct, taker_acct, .. } = t[0] {
        assert_eq!(qty.0, 5);
...
fn partial_fill_aggressor_rests_remainder() {
    ...
    let evs = book.apply(new_limit(2, ACCT_B, Side::Buy, 10010, 10)).into_vec();
...
fn partial_fill_maker_leaves_remainder() {
    ...
    let evs = book.apply(new_limit(2, ACCT_B, Side::Buy, 10010, 3)).into_vec();
...
fn price_time_priority_fifo_within_level() {
    ...
    let evs = book.apply(new_limit(4, ACCT_B, Side::Buy, 10010, 2)).into_vec();
    let t = trades(&evs);
    assert_eq!(t.len(), 2);

    if let EngineEvent::Trade { maker_order, maker_acct, .. } = t[0] {
        assert_eq!(maker_order.0, 1, "first fill should be seq=1 (ACCT_A)");
        assert_eq!(*maker_acct, ACCT_A);
    }
    if let EngineEvent::Trade { maker_order, maker_acct, .. } = t[1] {
...
fn best_price_priority_across_levels() {
    ...
    let evs = book.apply(new_limit(4, ACCT_B, Side::Buy, 10050, 5)).into_vec();
    let t = trades(&evs);
    assert_eq!(t.len(), 1);
...
fn cancel_resting_order() {
    ...
    let evs = book.apply(cancel(2, ACCT_A, 1 /*order_id == seq*/)).into_vec();
    assert!(evs.iter().any(|e| matches!(e, EngineEvent::Cancelled { order_id, .. } if order_id.0 == 1)));
...
fn cancel_updates_best_bid_to_next_level() {
    ...
    assert_eq!(book.best_bid(), Some(Price(10000)), "should fall back to 10000");
}
...
fn ioc_full_fill() {
    ...
    book.apply(new_limit(1, ACCT_A, Side::Sell, 10010, 5));
    let evs = book.apply(new_ioc(2, ACCT_B, Side::Buy, 10010, 5)).into_vec();
...
fn ioc_partial_fill_remainder_cancelled() {
    ...
    let evs = book.apply(new_ioc(2, ACCT_B, Side::Buy, 10010, 10)).into_vec();
    assert_eq!(trades(&evs).len(), 1);
    assert!(evs.iter().any(|e| matches!(e, EngineEvent::Cancelled { .. })));
...
fn ioc_no_match_rejected() {
    ...
    // No resting asks.
    let evs = book.apply(new_ioc(1, ACCT_A, Side::Buy, 10010, 5)).into_vec();
...
fn invalid_qty_rejected() {
    ...
    let evs = book.apply(new_limit(1, ACCT_A, Side::Buy, 10000, 0)).into_vec();
...
fn price_out_of_range_rejected() {
    ...
    // Below tick_floor.
    let evs = book.apply(new_limit(1, ACCT_A, Side::Buy, 5000, 10)).into_vec();
    assert_eq!(rejected(&evs).len(), 1);
}
...
fn sweep_multiple_levels() {
    ...
    let evs = book.apply(new_limit(4, ACCT_B, Side::Buy, 10050, 12)).into_vec();
...
fn book_top_event_emitted_after_resting() {
    ...
    let evs = book.apply(new_limit(1, ACCT_A, Side::Buy, 10000, 5)).into_vec();
    assert!(
        evs.iter().any(|e| matches!(e, EngineEvent::BookTop { bid: Some(Price(10000)), ask: None, .. })),
